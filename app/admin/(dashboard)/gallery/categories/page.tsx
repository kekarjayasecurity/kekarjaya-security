...
export default function AdminGalleryCategoriesPage() {
  ...
  function loadCategories() {
    fetch("/api/admin/gallery/categories").then((r) => r.json()).then((d) => { setCategories(Array.isArray(d) ? d : []); setLoading(false); }).catch(() => setLoading(false));
  }

  function openAdd() { setEditId(null); setName(""); setSlug(""); setModalOpen(true); }
  function openEdit(cat: GalleryCategory) { setEditId(cat.id); setName(cat.name); setSlug(cat.slug); setModalOpen(true); }

  async function handleSave() {
    if (!name || !slug) return;
    const url = editId ? `/api/admin/gallery/categories/${editId}` : "/api/admin/gallery/categories";
    const method = editId ? "PUT" : "POST";
    await fetch(url, { method, headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name, slug }) });
    setModalOpen(false);
    loadCategories();
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-primary-700">Kategori Galeri</h1>
